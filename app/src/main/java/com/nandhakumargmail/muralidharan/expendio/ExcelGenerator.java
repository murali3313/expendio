...
import static com.nandhakumargmail.muralidharan.expendio.Expense.*;
...
public class ExcelGenerator {
        ...
        for (Expenses dayWiseExpenses : monthWiseExpenses.getSortedDayWiseExpenses()) {
            ...
            if (dayWiseExpenses.size() > 1) {
                sheet.addMergedRegion(new CellRangeAddress(initialRows, initialRows + dayWiseExpenses.size() - 1,
                        headerColumns.size() - 1, headerColumns.size() - 1));
            }
            Cell dayWiseTotalCell = sheet.getRow(initialRows).getCell(headerColumns.size() - 1);
    ...
    private void createCellForValue(Workbook wb, Row row, Expense expense, int rowIndex, int cellIndex, int totalRowCount) {
        ...
        if (headerColumns.get(cellIndex).equals("Amount")) {
            c.setCellValue(Double.valueOf(expense.getValue(headerColumns.get(cellIndex))));
        } else {
            c.setCellValue(expense.getValue(headerColumns.get(cellIndex)));
        }

        if (cellIndex == 0) {
            oCellStyle.setBorderLeft(BorderStyle.THIN);
        }
        if (cellIndex == headerColumns.size() - 1) {
